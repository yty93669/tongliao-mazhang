...
const responsePriority = { HU: 3, MING_GANG: 2, PENG: 2, CHI: 1 };
function firstRoundXiAllowed(game, seat) {
    return game.players[seat].discarded.length === 0;
}
function openingXiPhase(game) {
...
function xiPatternForMeld(meld) {
    return xiDefs.find(def => def.name === meld.name)?.pattern;
}
function canGuoToMeld(game, meld, tile) {
    if (tile === 'Bai')
        return true;
...
function canGuoTile(game, seat, tile) {
...
function hasDrawnForGuo(game, seat) {
    return game.drawnTile?.seat === seat;
}
function firstGuoTile(game, seat) {
    if (!hasDrawnForGuo(game, seat))
        return undefined;
    const p = game.players[seat];
    return p.hand.find(t => canGuoTile(game, seat, t));
}
function targetGuoMeld(game, seat, tile) {
...
function removeOne(hand, tile) {
    ...
    if (i < 0)
        throw new Error('手里没有这张牌');
    hand.splice(i, 1);
}
function responseSeats(game) {
...
function responseActionsFor(game, seat) {
    const d = game.response ?? (game.lastDiscard ? { discardSeat: game.lastDiscard.seat, tile: game.lastDiscard.tile, source: game.lastDiscard.source, passed: [] } : undefined);
    if (!d || d.discardSeat === seat || d.passed.includes(seat))
        return [];
    const p = game.players[seat];
    const ctx = { fishTile: game.fishTile };
    const actions = [];
    if (isJiaHuWithTile(p.hand, d.tile, ctx).canHu)
        actions.push('HU');
    if (canMingGang(p.hand, d.tile))
        actions.push('MING_GANG');
    if (canPeng(p.hand, d.tile))
        actions.push('PENG');
    if (legalChiOptions(p.hand, d.tile, seat, d.discardSeat, d.source, ctx).length)
        actions.push('CHI');
    return actions;
}
function allPendingResponseChoices(game) {
    return responseSeats(game).flatMap(seat => responseActionsFor(game, seat).map(type => ({ seat, type, priority: responsePriority[type] })));
}
function seatDistanceAfter(discarderSeat, seat) {
    return (seat - discarderSeat + 4) % 4;
}
function assertResponseMayResolve(game, seat, type) {
    if (game.phase !== 'responding' || !game.response || !game.lastDiscard)
        throw new Error('当前没有可响应的弃牌');
    const ownActions = responseActionsFor(game, seat);
    if (!ownActions.includes(type))
        throw new Error(`不能${responseActionLabel(type)}`);
    ...
    if (samePriority[0]?.seat !== seat)
        throw new Error('还有座次更靠前的同优先级响应未处理');
}
...
function openResponseOrAdvance(game, discarderSeat, tile, source) {
    ...
    game.pendingXiSupplement = false;
    game.drawnTile = undefined;
    if (allPendingResponseChoices(game).length === 0)
        clearResponseAndAdvance(game);
}
function drawSupplement(game, seat, logType) {
...
function removeLastDiscardFromDiscardPile(game) {
    const d = game.lastDiscard;
    if (!d)
        throw new Error('没有可响应的弃牌');
    const discarder = game.players[d.seat];
    const i = discarder.discarded.lastIndexOf(d.tile);
    if (i >= 0)
        discarder.discarded.splice(i, 1);
    return d;
}
function finishClaim(game, seat) {
    game.response = undefined;
    game.lastDiscard = undefined;
    game.drawnTile = undefined;
    game.phase = 'playing';
...
export function availableActions(game, seat) {
    const p = game.players[seat];
    const ctx = { fishTile: game.fishTile };
    const actions = [];
    if (game.phase === 'betting') {
        if (seat === game.dealerSeat)
            actions.push('SET_BASE');
        if (!game.bets[seat]?.ready)
            actions.push('SET_BET');
        return actions;
    }
    if (game.phase === 'settlement') {
        if (!game.readyNext[seat])
    ...
    if (game.phase === 'responding') {
        ...
        if (responseSeats(game).includes(seat))
            actions.push('PASS');
    ...
    if (game.phase === 'playing' && game.currentSeat === seat) {
        ...
        if (openingXiPhase(game)) {
            if (xiSets.length)
                actions.push('DECLARE_XI');
            actions.push('END_XI');
            return [...new Set(actions)];
        }
        if (game.pendingXiSupplement && xiSets.length)
            actions.push('DECLARE_XI');
        if (game.pendingXiSupplement) {
            ...
            if (isWinningHand(p.hand, ctx).canHu)
                actions.push('HU');
            if (canBuGang(p.melds, p.hand))
                actions.push('BU_GANG');
            for (const t of p.hand)
                if (canAnGang(p.hand, t))
                    actions.push('AN_GANG');
            if (firstGuoTile(game, seat))
                actions.push('GUO');
        }
...
export function applyAction(game, action) {
    ...
    if (action.type === 'SET_BASE') {
        if (game.phase !== 'betting')
            throw new Error('当前不能设置基数');
        if (action.seat !== game.dealerSeat)
            throw new Error('只有庄家能设置基数');
        if (!Number.isFinite(action.baseScore) || action.baseScore <= 0)
            throw new Error('基数必须大于0');
    ...
    if (action.type === 'PASS') {
        if (game.phase !== 'responding' || !game.response)
            throw new Error('当前没有可过的响应');
        if (!responseSeats(game).includes(action.seat))
            throw new Error('该座位不能响应这张弃牌');
        if (!game.response.passed.includes(action.seat))
            game.response.passed.push(action.seat);
    ...
    if (action.type === 'DISCARD') {
        ...
        if (game.currentSeat !== action.seat)
            throw new Error('没轮到该玩家');
        if (openingXiPhase(game))
            throw new Error('请先等所有玩家结束开局亮喜');
        if (game.pendingXiSupplement)
            throw new Error('请先结束亮喜并补牌');
        if (!game.turnDrawn)
            throw new Error('必须先摸牌再打牌');
    ...
    if (action.type === 'DRAW') {
        ...
        if (game.currentSeat !== action.seat)
            throw new Error('没轮到该玩家');
        if (openingXiPhase(game))
            throw new Error('请先完成开局亮喜');
        if (game.pendingXiSupplement)
            throw new Error('请先结束亮喜并补牌');
        if (game.turnDrawn)
            throw new Error('已经摸过牌');
        game.lastDiscard = undefined;
        game.response = undefined;
        if (game.wall.length <= 4) {
            game.phase = 'fenzhang';
    ...
    if (action.type === 'DECLARE_XI') {
        if (game.phase !== 'playing')
            throw new Error('当前不能亮喜');
        if (game.currentSeat !== action.seat)
            throw new Error('没轮到该玩家');
        if (!firstRoundXiAllowed(game, action.seat))
            throw new Error('只有第一轮可以亮喜');
        if (!game.xiWindowOpen && !game.pendingXiSupplement)
            throw new Error('当前不能亮喜');
        const openingXi = openingXiPhase(game);
        const dealerOpening = openingDealerTurn(game, action.seat);
        if (!openingXi && !game.pendingXiSupplement)
            throw new Error('亮喜只能在开局摸第一张牌前进行');
        if (!game.pendingXiSupplement && !xiHandCountAllowed(game, action.seat))
            throw new Error('亮喜只能在开局摸第一张牌前进行');
        const xi = findXiSets(p.hand, { fishTile: game.fishTile }).find(x => x.name === action.name);
        if (!xi)
            throw new Error('没有这套喜');
        for (const t of xi.tiles)
            removeOne(p.hand, t);
    ...
    if (action.type === 'HU') {
        ...
        if (game.phase !== 'playing')
            throw new Error('当前不能胡');
        if (game.currentSeat !== action.seat)
            throw new Error('只有当前玩家能自摸胡');
        if (!game.turnDrawn || game.pendingXiSupplement)
            throw new Error('必须摸牌后才能自摸胡');
        if (!isWinningHand(p.hand, { fishTile: game.fishTile }).canHu)
            throw new Error('不能胡');
        game.phase = 'settlement';
        game.winnerSeat = action.seat;
    ...
    if (action.type === 'CHI') {
        ...
        if (!chosen)
            throw new Error('不能吃');
        removeLastDiscardFromDiscardPile(game);
        for (const t of chosen.filter(t => t !== d.tile))
            removeOne(p.hand, t);
    ...
    if (action.type === 'AN_GANG') {
        if (game.phase !== 'playing')
            throw new Error('当前不能暗杠');
        if (game.currentSeat !== action.seat)
            throw new Error('没轮到该玩家');
        if (!game.turnDrawn)
            throw new Error('必须先摸牌');
        const tile = action.tile ?? p.hand.find(t => canAnGang(p.hand, t));
        if (!tile || !canAnGang(p.hand, tile))
            throw new Error('不能暗杠');
        for (let i = 0; i < 4; i++)
            removeOne(p.hand, tile);
    ...
    if (action.type === 'GUO') {
        ...
        if (!game.turnDrawn)
            throw new Error('必须先摸牌');
        if (!hasDrawnForGuo(game, action.seat))
            throw new Error('必须抓牌后才能过牌');
        const tile = action.tile ?? firstGuoTile(game, action.seat);
        if (!tile || !canGuoTile(game, action.seat, tile))
            throw new Error('不能过这张牌');
        const meld = targetGuoMeld(game, action.seat, tile);
        if (!meld)
            throw new Error('没有可接过的喜');
        removeOne(p.hand, tile);
