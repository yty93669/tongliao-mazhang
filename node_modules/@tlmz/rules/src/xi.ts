import { countTiles, type TileCode } from './tiles.js';
import { matchesXiPatternToken, type FishContext } from './fish.js';
export type XiSet = { name: string; pattern: (TileCode|'Fish')[]; tiles: TileCode[] };
export const xiDefs: {name:string; pattern:(TileCode|'Fish')[]}[] = [
...
].map(([name, pattern]) => ({ name: name as string, pattern: pattern as (TileCode|'Fish')[] }));
export function findXiSets(hand: TileCode[], ctx: FishContext): XiSet[] {
  const counts = countTiles(hand); const out: XiSet[] = [];
  for (const def of xiDefs) {
    ...
    for (const token of def.pattern) {
      if (token === 'Fish') {
        if ((remaining.get(ctx.fishTile) ?? 0) <= 0) { ok = false; break; }
    ...
    if (ok) out.push({ name: def.name, pattern: def.pattern, tiles });
  }
  return out;
}
